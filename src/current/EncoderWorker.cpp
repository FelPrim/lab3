...
void EncoderWorker::initFFmpeg(int width, int height, int fps)
{
    ...
    if (!enc_codec) {
        emit errorOccurred("H.264 encoder (libx264) not found in linked libavcodec.");
        return;
    }
    ...
    m_enc_ctx->gop_size = fps * 2; // short GOP
    m_enc_ctx->max_b_frames = 0;   // no B-frames for low-latency
    m_enc_ctx->bit_rate = m_bitrate;
    m_enc_ctx->flags |= AV_CODEC_FLAG_LOW_DELAY;

    // thread count: hardware_concurrency - subtract (leave one for decoder)
    ...
    // encoder options: preset + tune for low-latency
    AVDictionary *enc_opts = nullptr;
    if (strcmp(enc_codec->name, "libx264") == 0 || strcmp(enc_codec->name, "h264") == 0) {
        av_dict_set(&enc_opts, "preset", DEFAULT_X264_PRESET, 0);
        av_dict_set(&enc_opts, "tune",   DEFAULT_X264_TUNE, 0);
        // force constant frame rate / low-latency params via x264-params
        // keyint = gop size, scenecut=0 to avoid long-latency scene cuts
        char x264params[256];
