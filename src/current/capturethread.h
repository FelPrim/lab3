...
void setBufferSeconds(int seconds) { 
        ...
        // Обновляем буфер если он существует
        if (m_packetBuffer) {
...
signals:
    ...
    void packetForDecoder(const QByteArray &packet);
...
private:
    ...
    EncoderWorker *m_encoderWorker = nullptr; // actually EncoderWorker*
    DecoderWorker *m_decoderWorker = nullptr; // actually DecoderWorker*
};
